...
public record Anchor(Vector2 pivot, Vector3 anchoredPosition)
{
    ...
    {
        return new Anchor(pivot, new Vector3(0f, 0f, 0f));
    }
}
...
internal class GeneralizedArtifact<T, V> : Artifact<T> where V : T where T : class
{
    ...
    public IEnumerator<T> GetEnumerator()
    {
        return Inner.Select(v => v as T).GetEnumerator();
    }
...
public abstract class GUIContext
{
    ...
    public static implicit operator GUIContextSupplier(GUIContext widget)
    {
        return () => widget ?? TORGUIContextEngine.Instance.EmptyContext;
    }
}
...
public interface GUI
{
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext LocalizedButton(GUIAlignment alignment, TextAttributes attribute, string translationKey, Action onClick,
    ...
    /// <param name="selectedColor">カーソルが重なっている時のボタンの色</param>
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext RawButton(GUIAlignment alignment, TextAttributes attribute, string rawText, Action onClick,
        Action onMouseOver = null, Action onMouseOut = null, Action onRightClick = null, Color? color = null,
        Color? selectedColor = null);
    ...
    /// <param name="selectedColor">カーソルが重なっている時のボタンの色</param>
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext Button(GUIAlignment alignment, TextAttributes attribute, TextComponent text, Action onClick,
        Action onMouseOver = null, Action onMouseOut = null, Action onRightClick = null, Color? color = null,
        Color? selectedColor = null);
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext ScrollView(GUIAlignment alignment, Size size, string scrollerTag, GUIContext inner,
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext HorizontalHolder(GUIAlignment alignment, IEnumerable<GUIContext> innerReference,
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext VerticalHolder(GUIAlignment alignment, float fixedWidth, params GUIContext[] inner)
    {
        return VerticalHolder(alignment, inner, null);
    }
    ...
    /// <param name="inner">並べるコンテキスト</param>
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext HorizontalHolder(GUIAlignment alignment, float fixedHeight, params GUIContext[] inner)
    {
        return HorizontalHolder(alignment, inner, null);
    }
    ...
    /// <param name="inner">並べるコンテキスト</param>
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext VerticalHolder(GUIAlignment alignment, params GUIContext[] inner)
    {
        return VerticalHolder(alignment, inner, null);
    }
    ...
    /// <param name="inner">並べるコンテキスト</param>
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext HorizontalHolder(GUIAlignment alignment, params GUIContext[] inner)
    {
        return HorizontalHolder(alignment, inner, null);
    }
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext VerticalMargin(float margin)
    {
        return Margin(new FuzzySize(null, margin));
    }
    ...
    /// <returns>生成されたコンテキスト定義</returns>
    GUIContext HorizontalMargin(float margin)
    {
        return Margin(new FuzzySize(margin, null));
    }
