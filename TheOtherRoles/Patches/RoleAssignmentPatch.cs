...
namespace TheOtherRoles.Patches
{
    ...
    class RoleManagerSelectRolesPatch
    {
        ...
        public static RoleAssignmentData getRoleAssignmentData()
        {
            ...
            crewSettings.Add((byte)RoleId.Marker, CustomOptionHolder.markerSpawnRate.getSelection());
        ...
        private static void assignModifiersToPlayers(List<RoleId> modifiers, List<PlayerControl> playerList, int modifierCount)
        {
            ...
            if (modifiers.Contains(RoleId.Lighterln))
            {
                int lighterlnCount = 0;
                while (lighterlnCount < modifiers.FindAll(x => x == RoleId.Lighterln).Count)
                {
                    playerId = setModifierToRandomPlayer((byte)RoleId.Lighterln, crewPlayer);
                    crewPlayer.RemoveAll(x => x.PlayerId == playerId);
                    playerList.RemoveAll(x => x.PlayerId == playerId);
                    lighterlnCount++;
                }
                modifiers.RemoveAll(x => x == RoleId.Lighterln);
            }
