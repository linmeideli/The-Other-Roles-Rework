...
internal static class MapBehaviourPatch
{
    ...
    public static void clearAndReload()
    {
        ...
        herePoints = new Dictionary<byte, SpriteRenderer>();
    }
    ...
    private static void Postfix(MapBehaviour __instance)
    {
        if (__instance.infectedOverlay.gameObject.active && PlayerControl.LocalPlayer.Data.IsDead &&
            CustomOptionHolder.deadImpsBlockSabotage.getBool()) __instance.Close();
        ...
        {
            foreach (var playerId in Trapper.playersOnMap)
            {
                if (herePoints.ContainsKey(playerId)) continue;
                var v = Trap.trapPlayerIdMap[playerId].trap.transform.position;
            ...
            {
                Object.Destroy(s.Value.gameObject);
                herePoints.Remove(s.Key);
            }
        }
        else if (Snitch.snitch != null && PlayerControl.LocalPlayer.PlayerId == Snitch.snitch.PlayerId &&
        ...
        if (PlayerControl.LocalPlayer.Data.IsDead && (!PlayerControl.LocalPlayer.Data.Role.IsImpostor ||
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                ...
                var pointName = $"TOR HerePoint {player.PlayerId}";
                var doublePoint = GameObject.Find(pointName);
                if (doublePoint != null) doublePoint.Destroy();
