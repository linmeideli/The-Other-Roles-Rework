...
static char* zip_strrpl(const char* str, size_t n, char oldchar, char newchar)
{
    char c;
    size_t i;
    char* rpl = calloc(1 + n, sizeof(char));
    char* begin = rpl;
    if (!rpl)
    {
        return NULL;
...
{
    ssize_t i = 0;
    mz_uint64* local_header_ofs_array = calloc(n, sizeof(mz_uint64));
                              ...
                              mz_uint64 read_num, size_t length)
{
    ...
    mz_uint8* move_buf = calloc(1, page_size);
    if (!move_buf)
    {
        return ZIP_EOOMEM;
    }
...
struct zip_t* zip_stream_open(const char* stream, size_t size, int level,
                              char mode)
{
    struct zip_t* zip = calloc(1, sizeof(struct zip_t));
    if (!zip)
    {
        return NULL;
    }
